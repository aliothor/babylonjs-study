...
export default class Animateparameter {
  engine: Engine;
  scene: Scene;

  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'Animate parameter'
  ...
  CreateScene(): Scene {
    ...
    this.engine.runRenderLoop(() => {
      ...
      ColorifyPluginMaterial.color.r = (timeElapsed / 5) % 1
    })
...
class ColorifyPluginMaterial extends MaterialPluginBase {
  ...
  bindForSubMesh(uniformBuffer: UniformBuffer, scene: Scene, engine: Engine, subMesh: SubMesh): void {
    if (this._isEnabled) {
      uniformBuffer.updateColor3('myColor', ColorifyPluginMaterial.color)
    }
  }
