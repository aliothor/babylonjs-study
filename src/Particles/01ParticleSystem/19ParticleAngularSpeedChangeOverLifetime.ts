...
export default class ParticleAngularSpeedChangeOverLifetime {
  engine: Engine;

  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>("h1")!.innerHTML =
      "Particle Angular Speed Change Over Lifetime";
  ...
  async CreateScene(): Promise<Scene> {
    ...
    // ptcSys.addAngularSpeedGradient(0, 0);
    // ptcSys.addAngularSpeedGradient(1, 12);

    for (let i = 0; i <= 2; i += 0.05) {
      ptcSys.addAngularSpeedGradient(i / 2, 30 * (1 - (i - 1) * (i - 1)));
    }
