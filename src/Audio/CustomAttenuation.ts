import { ArcRotateCamera, Color3, Curve3, DirectionalLight, Engine, FreeCamera, HemisphericLight, MeshBuilder, Scene, SceneLoader, Sound, StandardMaterial, Texture, Vector3 } from "babylonjs";

export default class CustomAttenuation {
  engine: Engine;
  scene: Scene;
  ...
  CreateScene(): Scene {
    ...
    const music1 = new Sound(
      ...
      scene,
      null,
      {autoplay: true, loop: true, useCustomAttenuation: true}
    );
    music1.attachToMesh(box);
    music1.setAttenuationFunction(
      function(curVolume, curDist, maxDist, refDist, rolloffFactor) {
        return (curVolume * curDist) / maxDist;
      }
    );
