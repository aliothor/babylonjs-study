...
export default class ShapeCast {
  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>("h1")!.innerHTML = "Shape Cast";
  }
  ...
  async CreateEngine(gpu: boolean = false): Promise<any> {
    if (gpu) {
  ...
  async CreateScene(engine: Engine): Promise<Scene> {
    ...
    mesh1.visibility = 0.6;
    mesh2.visibility = 0.6;
    const ray = new Ray(mesh1.position, new Vector3(1, 0, 0), 2.5);
      ...
      hk.shapeCast(
        {
          ...
          startPosition: mesh1.position,
          endPosition: mesh1.position.add(new Vector3(1, 0, 0).scale(2)),
