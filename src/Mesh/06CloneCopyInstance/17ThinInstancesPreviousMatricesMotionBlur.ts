...
export default class ThinInstancesPreviousMatricesMotionBlur {
  engine: Engine;
  scene: Scene;

  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'Thin Instances Previous Matrices Motion Blur'
  ...
  CreateScene(): Scene {
    ...
    const logicalParticles: { t: number; factor: number; speed: number; xFactor: number; yFactor: number; zFactor: number; mx: number; my: number; mat: Matrix; quat: Quaternion; pos: Vector3 }[] = []
    for (let i = 0; i < nbParticle; i++) {
      ...
      const data = { t, factor, speed, xFactor, yFactor, zFactor, mx: 0, my: 0, mat: new Matrix(), quat: new Quaternion(), pos: new Vector3() }
      logicalParticles.push(data)
    }

    const instancedBuffer = new Float32Array(16 * nbParticle)
    const previousInstanceBuffer = new Float32Array(16 * nbParticle)
    ...
    function updateParticles() {
      ...
      logicalParticles.forEach((particle, i) => {
        let { t, factor, speed, xFactor, yFactor, zFactor, mat, quat, pos } = particle
        ...
        Matrix.ComposeToRef(scl, quat, oldPos, mat)

        previousInstanceBuffer.set(mat.m, offset)
        offset += 16
      })

      poly.thinInstanceSetBuffer('matrix', instancedBuffer, 16)
      poly.thinInstanceSetBuffer('previosMatrix', previousInstanceBuffer, 16)
    }
