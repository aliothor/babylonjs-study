import { ArcRotateCamera, Color4, Engine, HemisphericLight, InstancedMesh, Matrix, MeshBuilder, Quaternion, Scene, Vector3 } from "babylonjs";

export default class InstancingAdvancedControl {
  engine: Engine;
  scene: Scene;

  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'Instancing Advanced Control'
  ...
  CreateScene(): Scene {
    ...
    const logicalParticles: { t: number; factor: number; speed: number; xFactor: number; yFactor: number; zFactor: number; mx: number; my: number; mat: Matrix; quat: Quaternion; }[] = []
    for (let i = 0; i < nbParticle; i++) {
      ...
      const data = { t, factor, speed, xFactor, yFactor, zFactor, mx: 0, my: 0, mat: new Matrix(), quat: new Quaternion() }
