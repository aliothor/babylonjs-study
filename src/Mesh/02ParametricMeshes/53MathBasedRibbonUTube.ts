...
export default class MathBasedRibbonUTube {
  engine: Engine;
  scene: Scene;

  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'Math Based Ribbon U-Tube'
  ...
  CreateScene(): Scene {
    ...
    const camera = new ArcRotateCamera('camera', -Math.PI / 2, Math.PI / 2.5, 35, new Vector3(0, 0, 0));
    ...
    for (let t = -6; t <= 6; t++) {
      ...
      for (let a = 0; a < 2 * Math.PI; a += Math.PI / 8) {
        const x = 2 * Math.cos(a) + t / 2
        const y = 2 * Math.sin(a) + t * t / 4
        const z = t
