import { ArcRotateCamera, AxesViewer, Engine, HemisphericLight, Mesh, MeshBuilder, Scene, Space, TransformNode, Vector3 } from "babylonjs";

export default class UseTransformNodeCenterRotation {
  engine: Engine;
  scene: Scene;

  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'Use TransformNode as a Center of Rotation'
  ...
  CreateScene(): Scene {
    ...
    scene.onBeforeRenderObservable.add(() => {
      ...
      pivot.position = pivot.position.add(axisNormal.scale(0.01 * sign))
