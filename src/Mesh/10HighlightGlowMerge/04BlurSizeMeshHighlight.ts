...
export default class BlurSizeMeshHighlight {
  engine: Engine;
  
  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'Blur Size Mesh Highlight'
  ...
  async CreateScene(): Promise<Scene> {
    ...
    scene.registerBeforeRender(() => {
      alpha += 0.05
      hl2.blurHorizontalSize = 0.9 + Math.cos(alpha)
      hl2.blurVerticalSize = 0.9 + Math.sin(alpha / 3)
    })
