...
export default class LODInstanceDemo {
  engine: Engine;
  
  constructor(private readonly canvas: HTMLCanvasElement) {
    document.querySelector<HTMLHtmlElement>('h1')!.innerHTML = 'LOD and Instance Demo'
  ...
  async CreateScene(): Promise<Scene> {
    ...
    knot1.setEnabled(false)
    const count = 3
    const scale = 4
    for (let x = -count; x <= count; x++) {
      for (let y = -count; y <= count; y++) {
          for (let z = 5; z < 10; z++) {
              const knot = knot1.createInstance("knotI")
